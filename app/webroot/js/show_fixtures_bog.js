...
const showFixtureByGw = (gameweek) => {
	...
	.then((results) => {
		...
		let matchesData = currenGwMatches.map((match) => {
		  ...
		  matchData.subgroupName = groupMap[match.entry1.group_id] + match.entry1.subgroup_id + ' vs ' + groupMap[match.entry2.group_id] + match.entry2.subgroup_id;
		...
		matchesData = matchesData.filter(function( element ) {
